...
#Sk_mean = moy_des_amps(S)
#print(Sk_mean[0])
print('mode_freq :',round(mode_freq_old(S),5), "Hz")
...
class Features:
    def __init__(self, filename, time, filearray,centroid,c_min_sdlow,c_plus_sdHi,sflatness,rms,maxfreq25):
...
for file in filelist:
    Stxt = read(file)
    #print ("centroid:", centroid(Stxt))
    #print ("c_min_sdlow:", c_min_sdlow(Stxt))
    #print("c_min_sdlow:",c_min_sdlow(Stxt))
    feature_objects.append(Features(file, timetxt(file), Stxt,
                                    centroid(Stxt),c_min_sdlow(Stxt),c_plus_sdHi(Stxt),
                                    sp_flatness(Stxt),rms(Stxt), 
                                    maxfreq25(Stxt)))
#print("file:",file)
print("time:",timetxt(file1))
#print("filearray",read(file))
#print("centroid:",centroid(file))
#print("c_min_sdlow:",c_min_sdlow(file))
...
for obj in sorted(feature_objects, key=attrgetter('time')):
    ...
    #print(obj.centroid)
    c_min_sdlow.append(obj.c_min_sdlow)
    c_plus_sdHi.append(obj.c_plus_sdHi)
...
# 1) ACP avec les données et avec les étiquettes (classes A, B, C, autres)
# 2) kmeans pour faire des groupes (> actuellement dans ce bloc)
# 3) autre mode de représentation : classification hierarchique => dendogramme "hierarchical clustering"
# 4) a voir k+proches voisins : methode de classement
# neurones profonds > methode de classementbcp bcp bcp d'exemples, qu'il faut générer (étiqueter) millions d'exemples

#for loop for feature extraction
from itertools import *
zip_list=[]
for i in zip(times, centroids,c_min_sdlow,c_plus_sdHi,sflatnesses,rmses,maxfreq25s):
    zip_list.append(i)
...
def get_distances(centroyd, points):
    """Returns the distance the centroid is from each data point in points."""
    return np.linalg.norm(points - centroyd, axis=1)
...
centroyds = initialize_clusters(df_std, k)
...
for i in range(maxiter):
    ...
    for i, c in enumerate(centroyds):
    ...
    for c in range(k):
        centroyds[c] = np.mean(df_std[classes == c], 0)
...
df.columns=['time','centroid','centre_ecartLow','centre_ecartHigh',
